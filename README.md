...
| # | 铁律 | 来源 | 违规后果 |
|---|------|------|---------|
| 1 | **锚定规格而非现状**：预期结果必须可追溯到 Rule citation，禁止照当前页面/接口现状写同义反复断言 | 维修能力 UI 测试教训 | 100% 通过 ≠ 用例好，意味着零信息量 |
| 2 | **拒绝路径要正向断言触发**：去重/权限/校验等后端合理拒绝，必须断言"正确拒绝"，不能换参数绕过 | 维修能力型号去重测试教训 | 绕过=掩盖缺陷 |
| 3 | **业务拒绝 ≠ 系统崩溃**：code=500 是 BusinessException 拒绝（非崩溃），code=-1 才是未捕获异常 | ai-products code 语义实测 | 用 `assertNotEquals(code, 500)` 会把正确拒绝误判为失败 |
| 4 | **异常场景 20%-40%**：每个功能点至少配 正向 + 异常/反向 + 边界 三件套 | MALL 卖场留货三次迭代经验 | 异常占比 <6% 是常见陷阱 |
